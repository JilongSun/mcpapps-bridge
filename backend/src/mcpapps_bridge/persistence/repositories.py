...
from .models import BridgeSessionRow, EndpointBindingRow, EndpointRow, UpstreamServerRow
    ...
    async def add(self, server: UpstreamServerDefinition) -> None:
        try:
            async with self._session_factory.begin() as session:
                session.add(_upstream_to_row(server))
...
class SqlAlchemyEndpointRepository:
    ...
    async def add(self, endpoint: EndpointDefinition) -> None:
        try:
            async with self._session_factory.begin() as session:
                session.add(_endpoint_to_row(endpoint))
                await session.flush()
                session.add_all(
...
async def seed_topology_if_empty(
    ...
    async with session_factory.begin() as session:
        ...
        if existing_upstream is not None or existing_endpoint is not None:
            return False
        session.add_all(_upstream_to_row(server) for server in upstream_servers)
        await session.flush()
...
def _bridge_session_to_row(bridge_session: BridgeSessionRecord) -> BridgeSessionRow:
    row = BridgeSessionRow(
        session_id=bridge_session.session_id, endpoint_id=bridge_session.endpoint_id
    )
