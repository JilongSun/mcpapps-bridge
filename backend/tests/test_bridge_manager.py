...
from mcpapps_bridge.repositories import EndpointRepository, UpstreamServerRepository
...
async def test_manager_marks_session_failed_when_core_session_cannot_start() -> None:
    ...
    async with manager.lifecycle():
        with pytest.raises(RuntimeError, match="Failed to connect to upstream MCP server"):
            await manager.open_session("fixture")
