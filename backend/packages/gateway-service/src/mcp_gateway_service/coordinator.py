"""Managed endpoints and isolated bridge session coordination."""
...
class GatewaySessionCoordinator:
    ...
    def default_endpoint(self) -> PublishedEndpoint:
        try:
            ...
            raise RuntimeError("GatewaySessionCoordinator has no published endpoints") from None
    ...
    async def load_published_endpoints(self) -> None:
        ...
        for revision in revisions:
            published = self._build_published_endpoint(revision)
            bindings = [binding for binding in published.revision.bindings if binding.enabled]
            upstreams = [binding.upstream.display_name for binding in bindings]
            logger.info(
